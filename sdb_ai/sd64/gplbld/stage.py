...
#
# Produces ONE directory, which is what an install consists of:
#
#   <stage>/SDCoreSolo/     ->  %USERPROFILE%\SDCoreSolo\
#
# 25 Sep 26 SD Core Solo (SOLO 2) - it produced two, ProgramFiles and
# ProgramData, for the multi-user layout.  Solo puts programs and data in one
# folder, and the two old roots never shared a name, so they are now the same
# directory.  The notes below that say "Program Files" or "ProgramData" describe
# the old split; the reasoning they record about usr\bin still holds.  NOTHING
# STAGED NAMES THE FOLDER: the programs find it from their own location
# (gplsrc/inipath.c), sd.conf holds no path and accounts/sdsys holds @SDSYS.
#
...
#
# /dev/shm is <root>\dev\shm, an empty directory staged for it.  There is no
# etc\fstab any more: it mapped /dev/shm out of read-only Program Files, and in
# Solo the whole tree belongs to the user.  Measured 25 Sep 2026: with no fstab
# the runtime resolves /dev/shm to <root>\dev\shm, and shm_open() works when the
# directory exists and fails when it does not - so it must be staged.  Without
# /dev/shm POSIX shared memory fails, which is the whole IPC layer (5.1).
#
...
SDSYS_EMPTY = [
    ...
    # 25 Sep 26 SD Core Solo (SOLO 4) - os.users and batch.jobs (and their
    # dictionaries) stood in this list.  Both were per-user permission lists on a
    # shared server; Solo has one user, and nothing reads either any more.
    # 02 Sep 26 - PRE_RELEASE_FIXES.md 28.  pdump() writes here instead of into
    # sdsys itself, which grants sdusers Modify and so let every SD user read
    # every other user's dumped variable state (pdump.c:98 falls back to
    # sysseg->sysdir when DUMPDIR is empty; SD_CONF below now sets it).
    #
    # secure-dumps.ps1 locks it once the tree ACL is on - write-only to
    # sdusers, on secure-audit.ps1's model.  NOT administrator-only: pdump is
    # in TIER.OMIT.STANDARD, so programmers have it, and k_error.c:286 fires it
    # unprompted on OptDumpOnError.  A directory they could not write would
    # lose the diagnostic exactly when something had already gone wrong.
    ('dumps',        'process dumps from pdump(); DUMPDIR points here'),
]
...
# 25 Sep 26 - dev/shm replaces shm: it is where the runtime looks with no fstab.
PROGRAM_DATA_DIRS = ['user_accounts', 'group_accounts', os.path.join('dev', 'shm')]

# 25 Sep 26 SD Core Solo - the one staged root, installed as
# %USERPROFILE%\SDCoreSolo (owner's ruling 1, 24 Sep 2026).
STAGE_ROOT = 'SDCoreSolo'
...
#
# 25 Sep 26 SD Core Solo - @SDSYS, NOT A PATH.  Solo's folder is the user's, so
# no build knows it.  Every reader of an account path expands the token -
# login (parse.pathname.tokens, three sites) and _voc_ref's Q-pointer lookup -
# through gpl.bp/pathtkn, which replaces a leading @SDSYS with the live @sdsys.
# The FIRST retarget, before the bootstrap, still writes the absolute staged
# path: that runs before pathtkn is catalogued.
PRODUCTION_SDSYS = '@SDSYS'
...
# 25 Sep 26 SD Core Solo - FSTAB is gone; /dev/shm is <root>\dev\shm with no
# mapping (see the header, and PROGRAM_DATA_DIRS).

# 25 Sep 26 SD Core Solo - NO PATH IN THIS FILE.  SDSYS, USRDIR and GRPDIR
# default to folders in SD's own folder (gplsrc/config.c), so they are not set
# here, and a line that set them would pin the file to one machine.  DUMPDIR
# is not set either: it was set only so a dump was not readable by the other
# SD users (PRE_RELEASE_FIXES.md 28), and Solo has none, so pdump.c:98's
# fallback to SDSYS is what it gets.
SD_CONF = """[sd]
GRPSIZE=2
NUMUSERS=20
SORTMEM=4096
...
; running UPDATE.ACCOUNTS on each account, which is what that verb is for.
'''
...
def main():
    ...
    # 25 Sep 26 SD Core Solo - pf and pd are the SAME directory now (see the
    # header).  Both names are kept so each block below still says which half
    # of the old layout it came from; nothing staged under one collides with
    # anything under the other.
    pf = os.path.join(stage, STAGE_ROOT)
    ...
    for f in PROGRAM_FILES_BIN_OPTIONAL:
        ...
        if os.path.isfile(src):
            ...
            shutil.copy2(src, dst)
            staged.add(stage, dst)
            print('  optional: %s staged to %s\\%s'
                  % (f, STAGE_ROOT, PF_BIN_SUBDIR))
        else:
    ...
    print('  micro: %d syntax file(s) staged to %s\\micro'
          % (len(os.listdir(os.path.join(mc_dst, 'syntax'))), STAGE_ROOT))
    print('  FILES_DICTS: %d dictionary records staged to %s\\gplbld'
          % (len(os.listdir(fd_dst)), STAGE_ROOT))
    ...
    if args.bootstrap:
        ...
        # installed system.
        #
        # 25 Sep 26 SD Core Solo - SD_CONF names no SDSYS, so the staged path is
        # ADDED after [sd] rather than substituted.  The staged sd.exe would
        # default to the same directory, but this build is not the place to
        # find out whether it does: name it.
        bconf = os.path.join(stage, 'bootstrap-sd.conf')
        if not SD_CONF.startswith('[sd]\n') or '\nSDSYS=' in SD_CONF:
            die('SD_CONF must begin "[sd]" and name no SDSYS - the bootstrap '
                'configuration is built by inserting one')
        with open(bconf, 'w', encoding='latin-1', newline='\r\n') as f:
            f.write(SD_CONF.replace('[sd]\n',
        ...
        staged.files = []
        staged.scan(stage, pf)  # pd is pf now - one scan, or every file twice
    ...
    print('  binaries in %s, so the POSIX root is the SD directory'
          % os.path.join(STAGE_ROOT, PF_BIN_SUBDIR))
    ...
    for _d, _files in CLIENT_DIRS:
        print('  %s: %s' % (os.path.join(STAGE_ROOT, PF_CLIENTS_SUBDIR, _d),
                            ' '.join(_files)))
