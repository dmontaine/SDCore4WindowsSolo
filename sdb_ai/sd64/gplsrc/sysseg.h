...
struct SYSSEG {
   u_int32_t revstamp;   /* SD revstamp that create the memory */
...
// MS 16 bits from from DEBUG configuration parameter
     ...
     #define SSF_SDFIX      0x00020000   /* Allow SDFix in interactive mode */
...
struct SEMAPHORE_ENTRY
 {
  int16_t owner;         /* SD user number of owner, zero if free, -ve for system processes */
...
/* 20240219 mab mods to handle AF_UNIX sockets */
  ...
  int16_t flags;                 /* Also defined in INT$KEYS.H */
     ...
     #define USR_SDAPISRVR   0x0004 /* Is sdApiSrvr process */
     #define USR_ADMIN       0x0008 /* Administrator privileges */
     #define USR_SDNET       0x0010 /* Is SDNet (USR_SDVBSRVR also set) */
