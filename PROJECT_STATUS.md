...
***THIS REPOSITORY STARTED FRESH ON 24 SEP 2026, WITH NO HISTORY.*** Owner's
instruction: `C:\Users\Don\SDCoreProject\SDCore4WindowsSolo`, pushed to
`dmontaine/SDCore4WindowsSolo` as ONE commit copied from `sd4windows` at
`36109e8e` (that repo's `main` the same day). **Every commit hash cited in this
file and in HISTORY.md is an `sd4windows` hash** — `git log` here will not find
it; read it in `C:\Users\Don\SDCoreProject\sd4windows` or on
`github.com/dmontaine/sd4windows`, which keeps the history and the `v1.0-0`/
`v1.1-0` tags. Paths written as `...\SDCoreProject\sd4windows\...` in these
files and in script help text are the old folder: use this one.
...
(SOLO 13, fixed — HISTORY.md). So the probe now drives `sd` NATIVELY (cmd, files, never pipes), like
